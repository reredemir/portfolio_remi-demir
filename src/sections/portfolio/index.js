        ...
        <Row>
          ...
          <Col md={10} className="recent-works">
            <div className="portfolio_selector">
              <button className="portfolio_category" onClick={() => this.changeCategory(null)}>
                <span className={`${!this.state.category ? 'active' : ''}`}>Tout</span>
              </button>
              {this.categories()}
            </div>
  ...
  items() {
    if (this.state.showPortfolio || this.context.height === 'auto') {
      ...
      return items.map((value, index) => {
        if (value.content.frontmatter.category === this.state.category || !this.state.category) {
    ...
    return categories.map((value, index) => {
      return (
        <button
          ...
          key={index}
        >
          <span className={`${this.state.category === value ? 'active' : ''}`}>{value}</span>
        </button>
      );
    });
  }
